            ...
            <div class="mx-auto max-w-7xl px-2 sm:px-6 lg:px-8">
                <div class="relative flex h-24 justify-between">
                    <div class="-ml-2 mr-4 flex items-center lg:hidden">
                        <!-- Mobile menu button -->
                        <button type="button" id="menuToggleButton" data-menu-is-open="false" class="relative inline-flex items-center justify-center rounded-md p-2 text-gray-400 hover:bg-maincolor-100 hover:text-gray-500 focus:outline-none" aria-controls="mobile-menu" aria-expanded="false">
                    ...
                    <div class="flex flex-1 items-center lg:items-stretch lg:justify-start">
                        ...
                        <div class="hidden lg:ml-20 lg:flex lg:space-x-8">
                            <?php foreach ($menuitems as $item) : ?>
                                <?php
                                $current_class = (is_page($item->object_id)) || (is_home() && $item->object_id == get_option('page_for_posts')) || (is_page($item->object_id) && !is_home() && !is_front_page($item->object_id)) || ($item->url === get_post_type_archive_link('post') && !is_front_page($item->object_id) && is_single()) ? 'border-b-2 border-secondarycolor text-secondarycolor' : 'border-b-2 border-transparent hover:border-gray-300 text-gray-500 hover:text-secondarycolor';
